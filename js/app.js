...
var storeApp = angular.module('GadgetMart',['ngRoute' , 'ngAnimate', 'initialValue']).
...
storeApp.controller('productCount', function ($scope, $http) {

    var totalProducts = $scope.store.products.length;

    var totalLaptop = 0;
    var totalEMachines = 0;
    var totalPhone = 0;
    var totalGatewayHardware = 0;

    for (var i = 0; i < totalProducts; i++) {

        if ($scope.store.products[i].category == "laptop")
            totalLaptop++;

        else if ($scope.store.products[i].category == "EMachine")
            totalEMachines++;

        else if ($scope.store.products[i].category == "phone")
            totalPhone++;

        else if ($scope.store.products[i].category == "Gateway Hardware")
            totalGatewayHardware++;
    }

    $scope.totalProducts = totalProducts;
    $scope.totalLaptop = totalLaptop;
    $scope.totalEMachines = totalEMachines;
    $scope.totalPhone = totalPhone;
    $scope.totalGatewayHardware = totalGatewayHardware;
...
storeApp.filter('startFrom', function () {
	...
	var myCart = new shoppingCart("AngularStore");
     myCart.addCheckoutParameters("PayPal", "aanchal23g@gmail.com");
